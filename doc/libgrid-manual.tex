...
\end{verbatim}
If the system has NVIDIA GPUs, libgrid can use CUDA to accelerate the grid operations. For Fedora-based systems, the propriatary NVIDIA drivers and the CUDA libraries can be obtained from: \url{https://negativo17.org/nvidia-driver/}. For Debian enable the testing repository in /etc/apt/sources.list so that you can install the latest version of CUDA. Both systems are compatible with the default settings in make.conf as described below. Currently, libgrid does not support AMD based GPUs.
...
Intel or Portland compilers have not been tested with libgrid.\\
...
REAL & Real floating point precision (float, double or quad precision).\\
     & Note that for libdft single precision floats tend to have limited use.\\
...
CUDA\_HOSTCC & GCC version that is compatible with the current installation of\\
             & CUDA (default is to auto detect; works for Fedora and Debian).\\
...
% make
\end{verbatim}
Once the utilities compile, install them (as root unless you changed ROOT above to point to your own directories):
\begin{verbatim}
# make install
\end{verbatim}
...
\end{verbatim}
These can be added to your .tcshrc (tcsh), .profile, or .bashrc file depending on your login shell. For the latter, the syntax is:
\begin{verbatim}
...
test.o: test.c
\end{verbatim}
This will compile the program specified in test.c and link the appropriate libraries automatically. Both CFLAGS and LDFLAGS are obtained automatically from libgrid's make.conf. If you used a different installation ROOT setting, replace /usr above with that.
