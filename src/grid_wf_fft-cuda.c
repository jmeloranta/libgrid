...
EXPORT char grid_cuda_wf_propagate_kinetic_fft(wf *gwf, REAL complex time_mass) {
  ...
  grid_cuda_wf_propagate_kinetic_fftW(cuda_find_block(grid->value), grid->kx0, grid->ky0, grid->kz0, grid->step, t, grid->nx, grid->ny, grid->nz);

 return 0;
}

/*
 * Propagate kinetic energy in Fourier space (with Lanczos filter).
 *
 * Only periodic boundaries implemented.
 *
 */

EXPORT char grid_cuda_wf_propagate_kinetic_cfft(wf *gwf, REAL complex time_mass, REAL kmax2) {
  ...
  grid_cuda_wf_propagate_kinetic_cfftW(cuda_find_block(grid->value), grid->kx0, grid->ky0, grid->kz0, grid->step, t, kmax2, grid->nx, grid->ny, grid->nz);
