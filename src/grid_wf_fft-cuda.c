...
EXPORT char grid_cuda_wf_propagate_kinetic_cfft(wf *gwf, REAL complex time_mass) {

  CUCOMPLEX t;
  cgrid *grid = gwf->grid;

  if(cuda_fft_policy(grid->value, grid->grid_len, grid->cuff_handle, grid->id) < 0) return -1;
