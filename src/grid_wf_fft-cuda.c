...
EXPORT char grid_cuda_wf_propagate_kinetic_fft(wf *gwf, REAL complex time_mass) {
  ...
  grid_cuda_wf_propagate_kinetic_fftW(cuda_block_address(grid->value), grid->kx0, grid->ky0, grid->kz0, grid->step, t, grid->nx, grid->ny, grid->nz);
...
EXPORT char grid_cuda_wf_propagate_kinetic_cfft(wf *gwf, REAL complex time_mass, REAL cnorm) {
  ...
  grid_cuda_wf_propagate_kinetic_cfftW(cuda_block_address(grid->value), grid->kx0, grid->ky0, grid->kz0, grid->step, t, cnorm, grid->nx, grid->ny, grid->nz);
