...
__global__ void grid_cuda_wf_propagate_kinetic_cfft_gpu(CUCOMPLEX *b, CUREAL norm, CUREAL cx, CUREAL cy, CUREAL cz, CUREAL kx0, CUREAL ky0, CUREAL kz0, CUREAL kmax2, CUCOMPLEX time_mass, INT nx, INT ny, INT nz, INT nyy, INT nx2, INT ny2, INT nz2, INT seg, INT il, INT iu, INT jl, INT ju, INT kl, INT ku) {
  ...
  kk = kx * kx + ky * ky + kz * kz;
  /* psi(k,t+dt) = psi(k,t) exp( - i (hbar^2 * k^2 / 2m) dt / hbar ) */
  if(kmax2 > 0.0 && kk > kmax2) b[idx] = CUMAKE(0.0, 0.0);
  else b[idx] = b[idx] * CUCEXP(time_mass * kk) * norm;
}
 ...
 * ky0      = Momentum shift of origin along Y (REAL; input).
 * kz0      = Momentum shift of origin along Z (REAL; input).
 * kmax     = Maximum allowed value for |k| (REAL; input). Wave vectors outside this radius will be set to zero. Set to zero to disable.
 * step     = Spatial step length (REAL; input).
 * time_mass= Time step & mass (CUCOMPLEX; input).
 * nx       = # of points along x (INT).
 * ny       = # of points along y (INT).
...
extern "C" void grid_cuda_wf_propagate_kinetic_cfftW(gpu_mem_block *grid, CUREAL norm, CUREAL kx0, CUREAL ky0, CUREAL kz0, CUREAL kmax, CUREAL step, CUCOMPLEX time_mass, INT nx, INT ny, INT nz) {
  ...
  for(i = 0; i < ngpu1; i++) {
    cudaSetDevice(GRID->GPUs[i]);
    grid_cuda_wf_propagate_kinetic_cfft_gpu<<<blocks1,threads>>>((CUCOMPLEX *) GRID->data[i], norm, cx, cy, cz, kx0, ky0, kz0, kmax2,
  ...
  for(i = ngpu1; i < ngpu2; i++) {
    cudaSetDevice(GRID->GPUs[i]);
    grid_cuda_wf_propagate_kinetic_cfft_gpu<<<blocks2,threads>>>((CUCOMPLEX *) GRID->data[i], norm, cx, cy, cz, kx0, ky0, kz0, kmax2,
