...
EXPORT char rgrid_cuda_function_operate_one(rgrid *dst, rgrid *src, rfunction *operator) {
  ...
  rgrid_cuda_function_operate_oneW(cuda_find_block(dst->value), cuda_find_block(src->value), cuda_find_block(operator->value), src->nx, src->ny, src->nz, operator->begin, operator->nsteps, operator->step);
...
EXPORT char rgrid_cuda_function_operate_one_product(rgrid *dst, rgrid *src1, rgrid *src2, rfunction *operator) {
  ...
  rgrid_cuda_function_operate_one_productW(cuda_find_block(dst->value), cuda_find_block(src1->value), cuda_find_block(src2->value), cuda_find_block(operator->value), 
