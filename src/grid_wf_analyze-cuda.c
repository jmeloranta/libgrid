...
EXPORT char grid_cuda_wf_velocity_x(wf *gwf, rgrid *vx, REAL inv_delta, REAL cutoff) {
  ...
  if(cuda_two_block_policy(grid->value, grid->grid_len, grid->cufft_handle, grid->id, 1, vx->value, vx->grid_len, vx->cufft_handle, vx->id, 0) < 0) return -1;
...
EXPORT char grid_cuda_wf_velocity_y(wf *gwf, rgrid *vy, REAL inv_delta, REAL cutoff) {
  ...
  if(cuda_two_block_policy(grid->value, grid->grid_len, grid->cufft_handle, grid->id, 1, vy->value, vy->grid_len, vy->cufft_handle, vy->id, 0) < 0) return -1;
...
EXPORT char grid_cuda_wf_velocity_z(wf *gwf, rgrid *vz, REAL inv_delta, REAL cutoff) {
  ...
  if(cuda_two_block_policy(grid->value, grid->grid_len, grid->cufft_handle, grid->id, 1, vz->value, vz->grid_len, vz->cufft_handle, vz->id, 0) < 0) return -1;
...
EXPORT char grid_cuda_wf_probability_flux_x(wf *gwf, rgrid *flux_x, REAL inv_delta) {
  ...
  if(cuda_two_block_policy(grid->value, grid->grid_len, grid->cufft_handle, grid->id, 1, flux_x->value, flux_x->grid_len, flux_x->cufft_handle, flux_x->id, 0) < 0)
    return -1;
...
EXPORT char grid_cuda_wf_probability_flux_y(wf *gwf, rgrid *flux_y, REAL inv_delta) {
  ...
  if(cuda_two_block_policy(grid->value, grid->grid_len, grid->cufft_handle, grid->id, 1, flux_y->value, flux_y->grid_len, flux_y->cufft_handle, flux_y->id, 0) < 0)
    return -1;
...
EXPORT char grid_cuda_wf_probability_flux_z(wf *gwf, rgrid *flux_z, REAL inv_delta) {
  ...
  if(cuda_two_block_policy(grid->value, grid->grid_len, grid->cufft_handle, grid->id, 1, flux_z->value, flux_z->grid_len, flux_z->cufft_handle, flux_z->id, 0) < 0) 
    return -1;
...
EXPORT char grid_cuda_wf_lx(wf *gwf, rgrid *workspace, REAL inv_delta) {
  ...
  if(cuda_two_block_policy(grid->value, grid->grid_len, grid->cufft_handle, grid->id, 1, workspace->value, workspace->grid_len, workspace->cufft_handle, workspace->id, 0) < 0)
    return -1;
