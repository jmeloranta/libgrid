...
EXPORT void grid_wf_incomp_KE(wf *gwf, REAL *bins, REAL binstep, INT nbins, rgrid *workspace1, rgrid *workspace2, rgrid *workspace3, rgrid *workspace4, rgrid *workspace5) {
  ...
  INT i, npts = gwf->grid->nx * gwf->grid->ny * gwf->grid->nz;
...
// TODO: Why do we need to divide by the number of points? Doing so matches previous work...
  for (i = 0; i < nbins; i++)
    bins[i] = bins[i] * (2.0 * M_PI) * (2.0 * M_PI) * (2.0 * M_PI) * 0.5 * gwf->mass / (4.0 * M_PI * (REAL) npts);
}
...
// TODO: Why do we need to divide by the number of points? Doing so matches previous work...
  for (i = 0; i < nbins; i++)
    bins[i] = bins[i] * 0.5 * gwf->mass * (2.0 * M_PI) * (2.0 * M_PI) * (2.0 * M_PI) / (4.0 * M_PI * (REAL) npts);
}
