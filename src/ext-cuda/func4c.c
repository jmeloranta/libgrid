...
#ifdef USE_CUDA
extern void grid_func4_cuda_operate_one_productW(gpu_mem_block *, gpu_mem_block *, gpu_mem_block *, CUREAL, CUREAL, CUREAL, INT, INT, INT);
extern void grid_func4_cuda_operate_oneW(gpu_mem_block *, gpu_mem_block *, CUREAL, CUREAL, CUREAL, INT, INT, INT);
#endif
...
#ifdef USE_CUDA
EXPORT char grid_func4_cuda_operate_one(rgrid *dst, rgrid *src, REAL beta, REAL rhom, REAL C) {
  ...
  grid_func4_cuda_operate_oneW(cuda_block_address(dst->value), cuda_block_address(src->value), beta, rhom, C, dst->nx, dst->ny, dst->nz);

  return 0;
}

EXPORT char grid_func4_cuda_operate_one_product(rgrid *dst, rgrid *src1, rgrid *src2, REAL beta, REAL rhom, REAL C) {
  ...
  if(cuda_three_block_policy(src2->value, src2->grid_len, src2->cufft_handle_r2c, src2->id, 1, src1->value, src1->grid_len, src1->cufft_handle_r2c, src1->id, 1, dst->value, dst->grid_len, dst->cufft_handle_r2c, dst->id, 0) < 0)
    return -1;
  grid_func4_cuda_operate_one_productW(cuda_block_address(dst->value), cuda_block_address(src1->value), cuda_block_address(src2->value), beta, rhom, C, dst->nx, dst->ny, dst->nz);

  return 0;
}
#endif

EXPORT void grid_func4_operate_one(rgrid *dst, rgrid *src, REAL beta, REAL rhom, REAL C) {
...
#ifdef USE_CUDA
  if(cuda_status() && !grid_func4_cuda_operate_one(dst, src, beta, rhom, C)) return;
...
#pragma omp parallel for firstprivate(nxy,nz,nzz,dvalue,svalue,beta,rhom,C) private(ij,ijnz,k) default(none) schedule(runtime)
  for(ij = 0; ij < nxy; ij++) {
    ijnz = ij * nzz;
    for(k = 0; k < nz; k++)
      dvalue[ijnz + k] = grid_func4(svalue[ijnz + k], beta, rhom, C);
  }
}

EXPORT void grid_func4_operate_one_product(rgrid *dst, rgrid *src1, rgrid *src2, REAL beta, REAL rhom, REAL C) {
...
#ifdef USE_CUDA
  if(cuda_status() && !grid_func4_cuda_operate_one_product(dst, src1, src2, beta, rhom, C)) return;
...
#pragma omp parallel for firstprivate(nxy,nz,nzz,dvalue,s1value,s2value,beta,rhom,C) private(ij,ijnz,k) default(none) schedule(runtime)
  for(ij = 0; ij < nxy; ij++) {
    ijnz = ij * nzz;
    for(k = 0; k < nz; k++)
      dvalue[ijnz + k] = s1value[ijnz + k] * grid_func4(s2value[ijnz + k], beta, rhom, C);
  }
}
